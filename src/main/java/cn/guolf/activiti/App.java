...
/**
 * 积分申报流程demo 包含功能:多实例会签、子流程并行审批、动态设置下一节点执行人员、任务超时自动完成
 *
 * @author guolf
 */
public class App {
  ...
  public static void main(String[] args) throws InterruptedException {
    ProcessEngineConfiguration cfg = new StandaloneProcessEngineConfiguration()
        .setJdbcUrl("jdbc:h2:mem:activiti;DB_CLOSE_DELAY=1000").setJdbcUsername("sa")
        .setJdbcPassword("")
    ...
    Deployment deployment = repositoryService.createDeployment()
        .addClasspathResource("MultiTask.bpmn")
        .name("流程测试")
        .category("")
        .deploy();
    ...
    ProcessInstance processInstance = runtimeService
        .startProcessInstanceByKey("myProcess", "Key001", vars);
    ...
    List<Task> taskList1 = taskService.createTaskQuery().taskAssignee("mary")
    ...
    List<Task> taskList2 = taskService.createTaskQuery().taskAssignee("jeck")
        .orderByTaskCreateTime().desc().list();
    ...
    List<Task> taskList3 = taskService.createTaskQuery().taskAssignee("tom").orderByTaskCreateTime()
        .desc().list();
    ...
    // 部门主任
    List<Task> taskListDept1 = taskService.createTaskQuery().taskAssignee("dept")
        .orderByTaskCreateTime().desc().list();
    ...
    // 市场专员
    List<Task> taskListSczy = taskService.createTaskQuery().taskAssignee("sczy")
        .orderByTaskCreateTime().desc().list();
    ...
    // 市场主任
    List<Task> taskListSczr = taskService.createTaskQuery().taskAssignee("sczr")
        .orderByTaskCreateTime().desc().list();
    ...
    // 财务专员
    List<Task> taskListCwzy = taskService.createTaskQuery().taskAssignee("cwzy")
        .orderByTaskCreateTime().desc().list();
    ...
    // 财务主任
    List<Task> taskListCwzr = taskService.createTaskQuery().taskAssignee("cwzr")
        .orderByTaskCreateTime().desc().list();
    ...
    List<Task> taskMe = taskService.createTaskQuery().taskAssignee("me").orderByTaskCreateTime()
        .desc().list();
    ...
    // 技术专员审批
    List<Task> taskListJishu = taskService.createTaskQuery().taskAssignee("jishu")
        .orderByTaskCreateTime().desc().list();
    System.out.println("taskListJishu = " + taskListJishu);
    taskService.complete(taskListJishu.get(0).getId());

    // 综合部公示
    List<Task> taskListZonghe = taskService.createTaskQuery().taskAssignee("zhb")
        .orderByTaskCreateTime().desc().list();
    System.out.println("taskListZonghe = " + taskListZonghe);
    taskService.complete(taskListZonghe.get(0).getId());

    // 公示确认，5秒未完成自动进入下一流程
    List<Task> taskListPublic = taskService.createTaskQuery().taskAssignee("张三")
        .orderByTaskCreateTime().desc().list();
    ...
    // 分管领导确认
    List<Task> taskListLeader = taskService.createTaskQuery().taskAssignee("leader")
        .orderByTaskCreateTime().desc().list();
